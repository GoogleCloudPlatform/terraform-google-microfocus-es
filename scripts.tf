...
sleep 10
until \
token=`gcloud compute instances get-guest-attributes ${local.blueprint_cluster_master_name} -q --zone ${local.zone} --query-path=blueprint/token --format="value(VALUE)" --quiet 2> /dev/null`
do sleep 10
done
echo $token
CMD
...
BEGIN {r=0;h="";t="";c=0}
{
cmd = "gcloud compute instances get-guest-attributes "$1" -q --zone "$2" --query-path=blueprint/install --format=\"value(VALUE)\" 2> /dev/null"
rv=""
rs=""
cmd | getline rv
cmd = "gcloud compute instances get-guest-attributes "$1" -q --zone "$2" --query-path=blueprint/install-status --format=\"value(VALUE)\" 2> /dev/null"
cmd | getline rs
if (rv == "") { rv = "booting" }
...
gcloud compute instances list -q --format="value(name,zone)" --filter="metadata['items']['key']=blueprint-role" | \
